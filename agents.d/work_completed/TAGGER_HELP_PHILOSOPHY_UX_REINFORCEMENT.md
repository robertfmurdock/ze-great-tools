...
- [x] Add a philosophy-first "fit check" layer to in-app help before option-level details
  - Agent cycle: test → implement → refactor-light → verify pushable
  - Update plan if guidelines revealed new constraints
- [x] Add guided "correct usage path" help that operationalizes the two-step workflow (`calculate-version` then `tag`)
  - Agent cycle: test → implement → refactor-light → verify pushable
  - Update plan if guidelines revealed new constraints
- [x] Improve failure and warning UX with direct remediation steps tied to Tagger principles
  - Agent cycle: test → implement → refactor-light → verify pushable
  - Update plan if guidelines revealed new constraints
- [x] Add Gradle plugin in-app discoverability and help parity for tasks and extension settings
  - Agent cycle: test → implement → refactor-light → verify pushable
  - Update plan if guidelines revealed new constraints
- [x] Add anti-pattern guardrails in help content to prevent common misuse patterns
  - Agent cycle: test → implement → refactor-light → verify pushable
  - Update plan if guidelines revealed new constraints
- [x] Add focused help-text regression tests that assert philosophy and workflow cues stay present
  - Agent cycle: test → implement → refactor-light → verify pushable
  - Update plan if guidelines revealed new constraints
  - Note: Tests were added incrementally with each feature slice (fit check, workflow, remediation, anti-patterns)
...
- Commands: 
  ...
  - `./gradlew :tools:tagger-plugin:check` (validation pass)
  - `./gradlew check` (full repository check pass)
  - `./gradlew tasks --group=versioning` (verified task descriptions visible)
- Results: All validation passed. Help text successfully reinforces philosophy and workflow at all touchpoints.
