...
For the simplest use, add the digger plugin to your Gradle root project as so:

```kotlin
plugins {
    id("com.zegreatrob.tools.digger") version "1.0.0"
}
...
The digger plugin adds a few tasks to your project.
...
By default, digger will look for `[none]`, `[patch]`, `[minor]`, and `[major]` in commit messages in order to determine the correct next version.
...
By default, digger will use a 'patch' version if it does not match any of the regexes. This behavior can be changed if you prefer "none":
...
Currently digger uses Grgit under the hood, so you can change the git username and password using its environment variables:
